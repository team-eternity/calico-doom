...
boolean P_TryMove (mobj_t *thing, fixed_t x, fixed_t y)
{
   ...
   if(movething)
   {
      if(thing->flags & MF_MISSILE)
      {
         ...
         P_DamageMobj(movething, thing, thing->target, damage);
      }
      else if(thing->flags & MF_SKULLFLY)
      {	
         /* skull bash into a monster */
         damage = ((P_Random()&7)+1) * thing->info->damage;
         P_DamageMobj(movething, thing, thing, damage);
      ...
      {
         /* pick up  */
         P_TouchSpecialThing(movething, thing);
      }
   }
