/*
  ...
  Moving object core implementation.
*/
...
//
// Remove an mobj from the world sim.
//
void P_RemoveMobj(mobj_t *mobj)
{
   ...
   // CALICO: clear targets
   P_SetTarget(&mobj->target,    NULL);
   P_SetTarget(&mobj->extramobj, NULL);

   // CALICO: do not unlink or free yet; set to deferred removal
   mobj->latecall = P_RemoveMobjDeferred;
}

//
// Respawn special items in deathmatch
//
...
//
// Set an mobj into a given state.
...
boolean P_SetMobjState(mobj_t *mobj, statenum_t state)
{
   ...
   if(mobj->latecall != P_RemoveMobjDeferred) // CALICO: if not removed...
...
//
// Blow up a missile when it hits something solid.
//
...
//
// Allocate a new mobj_t, populate it with its initial state, attach it to its
// proper position in the game world, and set it running.
//
...
//
// Spawn a thing at map start that was placed in DoomEd.
...
//
// Spawn a bullet puff from a hitscan attack.
//
void P_SpawnPuff(fixed_t x, fixed_t y, fixed_t z)
{
   mobj_t *th;

   z += ((P_Random() - P_Random()) << 10); // CALICO_FIXME: OoE
...
//
// Spawn blood when a thing is damaged.
//
...
//
// Spawn a new mobj which behaves as a damaging projectile.
//
...
//
// Spawn a new mobj as a player missile attack.
