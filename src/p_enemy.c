...
boolean P_Move(mobj_t *actor)
{
   ...
   if(!P_TryMove(actor, tryx, tryy))
   {
      ...
      if(P_UseSpecialLine(actor, blockline))
         good = true;
      return good;
   }
   else
...
newtarget:
      if(playeringame[1] && actor->target == players[0].mo)
         P_SetTarget(&actor->target, players[1].mo);
      else
...
void A_PosAttack (mobj_t *actor)
{
   ...
   angle  += (P_Random() - P_Random()) << 20; // CALICO_FIXME: OoE
...
void A_SPosAttack(mobj_t *actor)
{
   ...
   for(i = 0; i < 3; i++)
   {
      angle = bangle + ((P_Random()-P_Random())<<20); // CALICO_FIXME: OoE
      damage = ((P_Random()&7)+1)*3;
      P_LineAttack(actor, angle, MISSILERANGE, MAXINT, damage);
   }
}
...
void A_BossDeath(mobj_t *mo)
{
   ...
   {
      if(mo2->latecall == P_RemoveMobjDeferred) // CALICO: skip removed mobjs
         continue;
...
void L_MissileHit(mobj_t *mo)
{
   ...
   P_SetTarget(&mo->extramobj, NULL); // CALICO: done with reference
}
